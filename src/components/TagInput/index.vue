<template>
  <div :class="['tagInputarea', className]">
    <div ref="cmEle" :class="['tagInputareaIuput', 'ThemeBorderColor3', !!readonly ? 'readonlyBg' : '']"></div>
  </div>
</template>
...
onMounted(() => {
  if (cmEle.value) {
    ...
    //最小高度初始化
    if (props.minHeight) {
      let height = typeof props.minHeight === 'number' ? `${props.minHeight}px` : props.minHeight
      //编辑器高度
...
const cmBeforeChange = (cm: any, obj: any): any => {
  ...
  if (props.mode === MODE.FORMULA && (obj.origin === 'paste' || obj.origin === '+input' || obj.origin === '*compose')) {
    text = text.map((t: string) => t.replace(/[^+\-*\/0-9/a-z/A-Z\(\)\,\.]/gm, '').toUpperCase())
    // 最大输入10000字符
  ...
  if (props.readonly || (props.mode === MODE.ONLYTAG && obj.origin !== '+delete' && obj.origin !== 'inserttag' && obj.origin !== 'setValue')) {
    obj.cancel()
  }

  // 事件内，mode只能从每次props取，不然取不到最新
  if (props.mode === MODE.FORMULA && obj.origin !== '+delete' && obj.origin !== 'inserttag' && obj.origin !== 'setValue') {
    text = text.map((t: any) =>
      t
        .toUpperCase()
        .split('')
        .filter((t: any) => (obj.origin === 'paste' ? /[0-9A-Z\+\-\*\/\(\)\,\.\$]/ : /[0-9A-Z\+\-\*\/\(\)\,\.]/).test(t))
        .join('')
    )
  }
  if (props.mode === MODE.DATE && obj.origin !== '+delete' && obj.origin !== 'inserttag' && obj.origin !== 'setValue') {
    text = text.map((t: any) =>
      t
        .split('')
        .filter((t: any) => (obj.origin === 'paste' ? /[0-9YMdhm\+\-\$]/ : /[0-9YMdhm\+\-]/).test(t))
        .join('')
    )
  }
  obj.update(obj.from, obj.to, text)
...
const markColumns = (markers: any, value: any) => {
  ...
  poss.forEach((pos: any, i: number) => {
    renderColumnTag(pos.tag, { isLast: i === poss.length - 1 }, (node: any) => {
      markers.push(cmInstance.markText({ line: pos.line, ch: pos.start }, { line: pos.line, ch: pos.stop }, { replacedWith: node, handleMouseEvents: true }))
    })
  })
}
...
const markOperators = (markers: any, value: any) => {
  ...
  poss.forEach((pos: any, i: number) => {
    ...
    markers.push(cmInstance.markText({ line: pos.line, ch: pos.start }, { line: pos.line, ch: pos.stop }, { replacedWith: operatorEle, handleMouseEvents: true }))
  })
}
...
const insertColumnTag = (id: string) => {
  ...
  cmInstance.replaceRange(`${mode === MODE.FORMULA && autoComma && editorValue[position.ch - 1] === '$' ? ',' : ''}$${id}$`, position, undefined, 'inserttag')
  cmInstance.focus()
  cmInstance.execCommand('goDocEnd')
  if (cmEle.value) {
